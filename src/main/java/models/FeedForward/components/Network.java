...
public class Network {
    ...
    public void calculateNodeOutputs() {
        ...
        for (int i = 0; i < nodes_DEPRECATED.length; i++) { //goes through hidden and output need to separate out
            for (Node node : nodes_DEPRECATED[i]) {
    ...
    private void createOutputLayer(int size) {
        ...
        nodes_DEPRECATED[nodes_DEPRECATED.length - 1] = outputNodes; // TODO: remove
    ...
    private void setupConnections() {
        ...
        for (InputNode inputNode : inputLayer) {
            for (Node node : nodes_DEPRECATED[0]) { // This goes through each node in only the first layer
                new Connection(inputNode, node, randomWeight() + 0.1);
                if (count < 1) {
                    for (Node outputNeuron : nodes_DEPRECATED[nodes_DEPRECATED.length - 1]) {
